...
public class Main {
    static final String DB_CONNECTION = "jdbc:mysql://localhost:3307/mydbpro";
    ...
    public static void main(String[] args) {
        ...
        try {
            try {
                // create connection
                conn = DriverManager.getConnection(DB_CONNECTION, DB_USER, DB_PASSWORD); //получаем connection
                initDB();
    ...
    private static void initDB() throws SQLException {
        Statement st = conn.createStatement(); // передаем запрсоы через statment
        try {
    ...
    private static void addClient(Scanner sc) throws SQLException {
        ...
        //оформление запроса как шаблон                                                     //первое и второе значение
            ...
            try {
                ...
                try {
                    for (int i = 0; i < count; i++) {
                        ...
                        ps.executeUpdate(); //выполнение кода
                    }
                    conn.commit(); // завершение транзации
                } finally {
                    ps.close();
                }
            } catch (Exception ex) {
                conn.rollback(); //возврат на шаг назад при ошибке
            }
        } finally {
    ...
    private static void viewClients() throws SQLException {
        ...
        try {
            ...
            try {
                ...
                ResultSetMetaData md = rs.getMetaData(); //описывает получиный
...
//    private static void insertFlate(Scanner region, Scanner square,  )
}
